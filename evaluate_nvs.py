...
def task_1(efields, dir_output, model_type, action_type, dir_cache, split='test', creader=None):
    ...
    for vid in tqdm(valid_vids):
        ...
        for frame in frames:
            ...
            if True:
                ...
                sample['im_pred'] = (np.asarray(sample['im_pred']).astype(float) / 255).clip(0, 1)
                sample['im_targ'] = (np.asarray(sample['im_targ']).astype(float) / 255).clip(0, 1)
            ...
            f = compare_psnr(sample, mask_targ, psnr, psnr_f, psnr_b, action_type, vid, frame)
...
def main():
    ...
    task_1(efields, args.dir_output, args.model, args.action_type, args.dir_cache, split='test', creader=None)
